...
$query = mysql_query("SELECT *,date_format(data_falta, '%d/%m/%Y') AS data_falta FROM faltas INNER JOIN codigo_faltas ON codigo_faltas.id_faltas=faltas.id_falta INNER JOIN funcionario ON codigo_faltas.id=funcionario.id WHERE funcionario.id='$id_funcionario' " );
